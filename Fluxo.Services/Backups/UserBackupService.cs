...
public sealed class UserBackupService(IAppDataService appData) : IUserBackupService
{
    ...
    private async Task AppendExpensesAndLogsAsync(
        FluxoUserBackupDocument document,
        UserBackupSelection selection,
        Dictionary<int, int> sourceIdMap,
        Dictionary<int, int> tagIdMap,
        Dictionary<int, int> expenseIdMap,
        CancellationToken cancellationToken)
    {
        if (!selection.Includes(DataManagementEntityKind.Expenses))
            return;
    ...
    private async Task AppendIncomeLogsAsync(
        FluxoUserBackupDocument document,
        UserBackupSelection selection,
        Dictionary<int, int> sourceIdMap,
        CancellationToken cancellationToken)
    ...
    private async Task AddSelectedExpensesAsync(
        FluxoUserBackupDocument document,
        Dictionary<int, int> sourceBackupIds,
        Dictionary<int, int> tagBackupIds,
        Dictionary<int, int> expenseBackupIds,
        CancellationToken cancellationToken)
    {
        ...
        foreach (var expense in expenses)
        {
            ...
            var backupId = document.Entities.Expenses.Count + 1;
            expenseBackupIds[expense.Id] = backupId;
            document.Entities.Expenses.Add(new BackupExpense(
                backupId,
                sourceBackupId,
                tagBackupId,
                expense.Name,
    ...
    private async Task AddSelectedIncomeLogsAsync(
        FluxoUserBackupDocument document,
        Dictionary<int, int> sourceBackupIds,
        CancellationToken cancellationToken)
    {
        ...
        foreach (var log in logs)
        {
            ...
            document.Entities.IncomeLogs.Add(new BackupIncomeLog(
                document.Entities.IncomeLogs.Count + 1,
                sourceBackupId,
                log.Name,
                log.Amount,
                log.AddedOn,
