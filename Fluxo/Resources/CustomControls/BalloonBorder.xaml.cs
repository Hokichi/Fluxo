...
public partial class BalloonBorder : UserControl
{
    ...
    private void ApplyFill() => PART_Shape.Fill = BalloonFill;

    private void RebuildGeometry()
    {
        double w = ActualWidth;
        double h = ActualHeight;
        double c = CurveHeight;   // how far the sides puff out
        double r = CornerRadius;

        if (w <= 0 || h <= 0) return;

        // Keep content inset so it clears the bulging edges
        double inset = c + r * 0.5;
        ...
        figure.Segments.Add(new BezierSegment(new Point(w * 0.25, -c), new Point(w * 0.75, -c), new Point(w - r, 0), true));
        // Top-right corner
        ...
        figure.Segments.Add(new BezierSegment(new Point(w + c, h * 0.25), new Point(w + c, h * 0.75), new Point(w, h - r), true));
        // Bottom-right corner
        ...
        figure.Segments.Add(new BezierSegment(new Point(w * 0.75, h + c), new Point(w * 0.25, h + c), new Point(r, h), true));
        // Bottom-left corner
